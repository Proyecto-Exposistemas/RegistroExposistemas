...
if(isset($_POST["NumeroActividad"])){
    ...
    $sql_evento = "SELECT * FROM evento WHERE no_evento=:numero";
    $parametros_eventos = [":numero"=>$no_evento];
    $resultado_evento = $conexion->MOSTRAR($sql_evento,$parametros_eventos);

    $sql_maestros = "SELECT  docentes.nombre, docentes.paterno, docentes.materno, asesores_evento.rfc 
                        FROM asesores_evento, docentes WHERE asesores_evento.no_evento = :numero 
...
$data = ["mensaje"=>$mensaje_evento, "evento"=>$lista_evento, "alumnos"=>$lista_alumnos,"alumnos_llaves"=>$lista_alumnos_llaves,
        "mestros"=>$lista_maestros, "maestros_llaves"=>$lista_maestros_llaves];
